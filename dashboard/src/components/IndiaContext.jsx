...
const IndiaContext = ({ waterMl = 0 }) => {
  const bottles = (waterMl / 500);
  const showAsBottles = waterMl > 500;
  // For bottle fill visualization, cap at 100%
  const fillPercent = showAsBottles ? 100 : (waterMl / 500) * 100;

  return (
    <div className="india-context glass-card">
      <div className="context-header">
        <h3>India Water Context</h3>
        <span className="context-badge">Reference Comparison</span>
      </div>
        ...
        <div className="context-visual">
          <div className="bottle-container">
            <svg viewBox="0 0 40 100" className="bottle-svg">
              ...
              <rect x="11" y={90 - (90 * (Math.min(100, fillPercent) / 100))} width="18" height={90 * (Math.min(100, fillPercent) / 100)} 
            ...
            <div className="comparison-text">
              {showAsBottles ? (
                <>
                  ...
                  <span className="highlight">{bottles.toFixed(1)}</span> × 500 mL bottles
                </>
              ) : (
                <>
                  <span className="highlight">{waterMl.toLocaleString()} mL</span> ≈{' '}
                  <span className="highlight">{fillPercent.toFixed(1)}%</span> of a 500 mL bottle
                </>
              )}
            </div>
            <p className="context-description">
              Estimated aggregate water footprint of AI usage in this period.
              In a country where millions face water scarcity, understanding AI's resource cost matters.
            </p>
          </div>
        </div>
        
        <div className="context-note">
          <h4>Historical Context (Chennai, 2019)</h4>
          <p>
          ...
          <div className="note-disclaimer">
            *Historical/reference context. This is static, bundled reference data — not live or real-time.
          </div>
        </div>
      </div>
    </div>
