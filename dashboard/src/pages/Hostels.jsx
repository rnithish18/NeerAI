...
import { fetchHostelStats } from '../api';
...
const Hostels = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setHostels(await fetchHostelStats());
    };
    loadData();
  }, []);

  if (hostels.length === 0) return <div>Loading...</div>;
  ...
  return (
    <div className="page animate-fade-in">
      ...
      <div className="glass-card mb-6" style={{ marginBottom: '1.5rem', height: '400px' }}>
        <h3 style={{ marginBottom: '1.5rem' }}>Estimated Energy Consumption by Hostel (kWh)</h3>
        <ResponsiveContainer width="100%" height="90%">
          <BarChart data={hostels} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {hostels.map((hostel) => (
          <div key={hostel.name} className="glass-card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div style={{ background: 'var(--bg-secondary)', padding: '1rem', borderRadius: '8px' }}>
              ...
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--accent-water)' }}>
                {(hostel.water / 1000).toFixed(1)} <span style={{ fontSize: '1rem' }}>L</span>
              </div>
            </div>
            ...
            <div style={{ background: 'var(--bg-secondary)', padding: '1rem', borderRadius: '8px' }}>
              ...
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--accent-warning)' }}>
                {hostel.energy.toFixed(1)} <span style={{ fontSize: '1rem' }}>kWh</span>
              </div>
            </div>
          </div>
        ))}
